...
def get_adj(edges, size):
    ...
    adj = torch.sparse_coo_tensor(edges_tensor, values, size=size, dtype=torch.long).to_dense().long()
    return adj
...
def get_data(args):
    ...
    drp = pd.read_csv(args.data_dir + 'DrugTargets.csv').iloc[:, 1:].to_numpy()
    ...
    # print(data['drugfeature'].shape) (663, 300)
    # print(data['diseasefeature'].shape)(409, 64)
    # print(data['proteinfeature'].shape)(993, 320)
   

    # data['drugfeature'] = np.random.rand(663, 300)
    # data['diseasefeature'] = np.random.rand(409, 64)
    # data['proteinfeature'] = np.random.rand(993, 320)
...
def data_processing(data, args):
    ...
    # b = 2 / 5
    # c = 1 / 5
    # print(data['drf'].shape)
    # print(data['drg'].shape)
    # print(data['drp'].shape)
    # drs_mean = (a * data['drf'] + b * data['drg'] + c * data['drp'])
    # drs_mean = (data['drg'] + data['drp']) / 2
    weight_drf = 1
    weight_drg = 1
    weight_drp = 0
    drs_mean = (weight_drf * data['drf'] + weight_drg * data['drg'] + weight_drp * data['drp']) / (weight_drf + weight_drg + weight_drp)
